...
import { processPaymentWithMercadoPago, initMercadoPagoIntegration } from './mercadopago-integration.js';
...
export function renderOrderSummary() {
  ...
  shippingEl.textContent = summary.shipping === 0 ? 'Grátis' : formatCurrency(summary.shipping);
...
function validateForm() {
  ...
  // WhatsApp
  const whatsapp = document.getElementById('input-whatsapp').value.replace(/\D/g, '');
...
function collectFormData() {
  return {
    ...
    whatsapp: document.getElementById('input-whatsapp').value.replace(/\D/g, ''),
...
function generateWhatsAppMessage() {
  ...
  message += `Frete: ${summary.shipping === 0 ? 'GRÁTIS' : formatCurrency(summary.shipping)}\n`;
...
async function processWithMercadoPago() {
  ...
  try {
    const result = await processPaymentWithMercadoPago(customerData, shippingData);
